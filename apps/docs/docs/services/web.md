...
- **Выходы:** визуализация прогноза, explainability по факторам, UI-состояния ошибок при недоступности погодных данных.
...
- **Same-origin weather proxy:**
  ...
  - Целевой UX-контракт: UI должен отображать разные пользовательские состояния для этих статусов (например, «неправильные координаты» vs «провайдер недоступен») и не строить synthetic-прогноз.
...
- поведение внешних погодных данных контролируется через same-origin proxy и явное error-state без synthetic weather;
...
- Изменить weather flow: сначала same-origin route и нормализация, затем UI error-режимы без synthetic weather.
