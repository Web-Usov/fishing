...
- для прогноза: `domain-*` остаются framework/contract-agnostic, а `shared-zod` используется на транспортных границах.
...
7. Не дублировать контрактные типы в app-слоях: использовать shared-контракты как единственный источник истины.
8. `packages/shared-zod` содержит только boundary-контракты/схемы и константы валидации; доменная бизнес-логика и domain-инварианты в этот пакет не добавляются.
